...
$get_p_order="select * from customer_order";
$run_p_order=mysqli_query($con,$get_p_order);
$count_order=mysqli_num_rows($run_p_order);
  ?>
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
  ...
  <body>
    ...
    <div class="wrapper">
     ...
     if (isset($_GET['return_payment'])) {
          $payment_id = $_GET['return_payment'];
          ...
          // Ambil order terkait berdasarkan invoice_id
          $get_order = "SELECT * FROM customer_order WHERE invoice_no='$invoice_id'";
          ...
          // Update status pembayaran menjadi unpaid di customer_order
          $update_order = "UPDATE customer_order SET order_status='unpaid' WHERE invoice_no='$invoice_id'";
          $run_update_order = mysqli_query($con, $update_order);
      
          // Kembalikan stok produk ke jumlah semula
          $update_product_stock = "UPDATE products SET stok=stok + $qty WHERE product_id='$product_id'";
          $run_update_product_stock = mysqli_query($con, $update_product_stock);
      
          // Hapus pembayaran dari tabel payments
          $delete_payment = "DELETE FROM payments WHERE payment_id='$payment_id'";
          $run_delete_payment = mysqli_query($con, $delete_payment);
      
          if ($run_update_order && $run_update_product_stock && $run_delete_payment) {
              echo "<script>alert('Payment has been returned and product stock updated.');</script>";
              echo "<script>window.open('index.php?view_payments', '_self');</script>";
          } else {
              echo "<script>alert('Failed to return payment.');</script>";
          }
      }
